...
if ($_POST['action'] === 'update_service') {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT is_system_service, service_name FROM services WHERE service_id = ?");
        $stmt->execute([$id]);
        $service = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($service && $service['is_system_service'] == 1) {
            echo json_encode([
                'success' => false, 
                'message' => '🔒 Cannot edit "' . $service['service_name'] . '". This is a system-protected service.'
            ]);
            exit;
        }
...
if ($_POST['action'] === 'delete_service') {
    ...
    try {
        ...
        if ($service && $service['is_system_service'] == 1) {
            echo json_encode([
                'success' => false, 
                'message' => '🔒 Cannot delete "' . $service['service_name'] . '". This is a system-protected service.'
            ]);
            exit;
        }
...
<div class="container">
    ...
    <tr>
        ...
        <td>
            ...
            <?php if ($service['is_system_service'] != 1): ?>
            ...
            <?php else: ?>
                <button class="btn-edit" style="background: #9ca3af; cursor: not-allowed;" disabled title="System service - cannot edit">
                    <i class="fas fa-lock"></i> Protected
                </button>
                <button class="btn-danger" style="background: #9ca3af; cursor: not-allowed;" disabled title="System service - cannot delete">
