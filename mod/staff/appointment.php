    ...
    try {
        ...
        if ($stmt_update->affected_rows > 0) {
            ...
            if ($newStatusName === 'Confirmed' && !empty($client_email)) {
                ...
                // ==================================================
                $qr_data_lines = [];
                $qr_data_lines[] = "APPOINTMENT ID: " . $id;
                $qr_data_lines[] = "PATIENT: " . htmlspecialchars($client_name);
                if (!empty($service_name)) { $qr_data_lines[] = "SERVICE: " . htmlspecialchars($service_name); }
                $qr_data_lines[] = "DATE: " . $formatted_date;
                $qr_data_lines[] = "TIME: " . $formatted_time;
                if (!empty($client_email)) { $qr_data_lines[] = "EMAIL: " . htmlspecialchars($client_email); }
                $qr_data = implode("\n", $qr_data_lines);
...
<head>
    ...
    <style>
        ...
        .header { background: #1d4ed8; /* === BAGONG KULAY (BLUE) === */ padding: 30px 20px; text-align: center; }
...
.flatpickr-day.has-appointments {
    ...
    border-color: #1d4ed8; /* Theme blue */
    color: #1e40af; /* Dark blue */
    font-weight: bold;
}
.flatpickr-day.has-appointments:hover { background: #c7d2fe; }
...
/* === BAGONG KULAY (BLUE) === */
.btn-filter.active { background: #1d4ed8; color: #fff; border-color: #1d4ed8; }
.stats { 
...
/* === BAGONG KULAY (BLUE) === */
.detail-header { 
    background: linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%); padding: 24px 28px; 
...
<div id="main-content" style="display: none;">
              ...
              <tr style="border-bottom:1px solid #f3f6f9;" data-id="<?= $appt['appointment_id'] ?>" data-status="<?= strtolower($appt['status_name']) ?>">
                ...
                <td style="padding:12px 8px;vertical-align:middle;">
                  <div style="display:flex;align-items:center;gap:10px;">
                    <div style="width:40px;height:40px;border-radius:50%;background:#1d4ed8;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:800; flex-shrink: 0;">
    ...
    <div id="actionLoader" class="detail-overlay" style="z-index: 9990;" aria-hidden="true">
        <div class="loader-card" style="background: #fff; border-radius: 12px; padding: 24px; display: flex; align-items: center; gap: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.3);">
            <div class="loader-spinner" style="border-top-color: #1d4ed8; width: 32px; height: 32px; border-width: 4px; flex-shrink: 0;"></div>
