...
.vertical-bar {
    ...
    width: 55px;
    height: 100vh;
    background: linear-gradient(180deg, #1e3a8a 0%, #1d4ed8 100%); /* <-- THEME BLUE */
    z-index: 1000;
}
.vertical-bar .circle {
    width: 70px;
    ...
    border: 4px solid #1e3a8a; /* <-- THEME BLUE */
...
.close-btn { 
    background: #dc3545; /* Kept red for semantic meaning */
    color: #fff; 
    border: none; 
    padding: 10px 20px;
    border-radius: 6px;
...
.card .change.negative {
    color: #e74c3c; /* Kept red for semantic meaning */
}
.charts-grid {
    display: grid;
...
.detail-header { background: linear-gradient(135deg, #1e3a8a 0%, #1d4ed8 100%); padding: 24px 28px; border-radius: 16px 16px 0 0; display: flex; justify-content: space-between; align-items: center; } /* <-- THEME BLUE */
...
.loader-spinner {
    ...
    border-top: 5px solid #1d4ed8; /* <-- THEME BLUE */
    border-radius: 50%;
    animation: spin 1s linear infinite;
    margin-bottom: 15px;
}
...
const weeklyColors = weeklyValues.map((val, idx) => idx % 2 === 0 ? '#1d4ed8' : '#27ae60'); /* <-- THEME BLUE */
...
new Chart(ctx1, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Appointments',
            data: dailyValues,
            borderColor: '#1d4ed8', /* <-- THEME BLUE */
            backgroundColor: 'rgba(29, 78, 216, 0.1)', /* <-- THEME BLUE */
            borderWidth: 2,
            fill: true,
            tension: 0.4,
            pointRadius: 4,
            pointBackgroundColor: '#1d4ed8' /* <-- THEME BLUE */
        }]
    },
    options: {
        responsive: true,
