<?php
// =======================================================
// UPDATED: Admin Profile with Strong Password, Real-Time Validation, & Page Loader
// =======================================================
session_start();
...
if (isset($_POST['action'])) {
            ...
            $mail->Body    = "
                <div style='font-family:sans-serif; padding:20px; background:#f4f4f4;'>
                    <div style='background:#fff; padding:20px; border-radius:8px; max-width:500px; margin:0 auto; border-top:5px solid #991010;'>
                        ...
                        <p>You requested to change your admin email address. Please use the verification code below to complete the process:</p>
    ...
    if ($action === 'updateProfile') {
        ...
        try {
            ...
            $updateStmt = $conn->prepare("UPDATE admin SET name=?, email=?, password=? WHERE id=?");
...
emailInput.addEventListener('input', function() {
    ...
    emailTimer = setTimeout(() => {
        fetch('profile.php', {
            ...
            if(res.exists) {
                msg.innerHTML = '<span style="color:#dc2626;">❌ Email is already in use by another admin</span>';
                isEmailValid = false;
            } else {
...
function confirmLogout() {
  showConfirm("Are you sure you want to log out of your admin account?", "Yes, Logout")
    .then(confirmed => {
      if (confirmed) {
