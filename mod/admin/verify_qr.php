...
// ===== DATABASE QUERY =====
try {
    // UPDATED QUERY: Nag-join tayo sa clients at users table para makuha ang email at address
    $sql = "SELECT 
                a.*, 
                st.status_name, 
                ser.service_name, 
    ...
    if ($appointment) {
        ...
        if ($status === 'Confirmed' || $status === 'Missed') {
            ...
            // ============================================
            // **I-DECRYPT ANG SENSITIVE DATA**
            // ============================================
            // Fallback kapag walang pangalan sa appointment table
            if (empty($appointment['full_name'])) {
                $appointment['full_name'] = !empty($appointment['user_full_name']) ? $appointment['user_full_name'] : '';
            }
            ...
            // Decrypt new fields
            if (!empty($appointment['email'])) $appointment['email'] = decrypt_data($appointment['email']);
            if (!empty($appointment['address'])) $appointment['address'] = decrypt_data($appointment['address']);

            // Decrypt other medical/text fields
            ...
            if (!empty($appointment['notes'])) $appointment['notes'] = decrypt_data($appointment['notes']);
            if (!empty($appointment['previous_color_issues'])) $appointment['previous_color_issues'] = decrypt_data($appointment['previous_color_issues']);
            if (!empty($appointment['ishihara_notes'])) $appointment['ishihara_notes'] = decrypt_data($appointment['ishihara_notes']);
            if (!empty($appointment['color_issues'])) $appointment['color_issues'] = decrypt_data($appointment['color_issues']);
            if (!empty($appointment['ishihara_reason'])) $appointment['ishihara_reason'] = decrypt_data($appointment['ishihara_reason']);
            if (!empty($appointment['certificate_purpose'])) $appointment['certificate_purpose'] = decrypt_data($appointment['certificate_purpose']);
            if (!empty($appointment['certificate_other'])) $appointment['certificate_other'] = decrypt_data($appointment['certificate_other']);
