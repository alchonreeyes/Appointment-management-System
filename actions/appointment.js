...
async function checkSlots() {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (remaining > 0) {
        remainingDisplay.textContent = `✅ ${remaining} slot${remaining !== 1 ? 's' : ''} available (across all services)`;
        ...
        alert("Sorry, this date is fully booked for all services. Please choose another date.");
      }
    } else {
